...
def save_predictions(model, model_name, file_paths):
    ...
    with torch.no_grad():
        ...
        for x_batch, y_batch, file_path_batch in dataloader:
            scores = model(x_batch)
            for fp, scr in zip(file_path_batch, scores.detach().cpu().tolist()):
    ...
    {'name': 'Wind Farm C', 'number': 33,
     'include':
        [11, 33, 44, 49, 31, 67, 9, 91, 5, 90, 70, 35, 16,
         ...
         80, 37, 29, 1, 20, 60],
     'threshold': {'MSELoss': 0.89, 'WeightedAnomalyLoss': 0.9996, 'RewardedAnomalyLoss': 0.6}},
]
criteria = [
    'MSELoss',
    # 'WeightedAnomalyLoss',
...
for config in configs:
    for loss in criteria:
        ...
        print(model_name)
        calc_care(file_paths, model_name, threshold)
